...
class BlogActivityTests(TestCase):

    def test_activity_with_type_created_is_created_when_post_is_created(self):
        """
        Test a "Created" type activity gets created when a new post is saved.
        """
        reset_db()
        p = create_post()
        expected = 1
        number_of_activities = SiteActivity.objects.count()
        expected_id = p.id
        expected_title = p.title
        expected_type = 'Created'
        sa = SiteActivity.objects.get()
        self.assertEqual(number_of_activities, expected, 'Expected %s activity, got %s' % (expected, number_of_activities))
        self.assertEqual(sa.post_id, expected_id, 'Expected matching ids, got %s vs %s' % (expected_id, sa.post_id))
        self.assertEqual(sa.post_title, expected_title, 'Expected matching titles, got %s vs %s' % (expected_title, sa.post_title))
        self.assertEqual(sa.task, expected_type, 'Expected "%s" task, got %s' % (expected_type, sa.task) )

    def test_activity_with_type_updated_is_created_when_post_is_updated(self):
        """
        Test an "Updated" type activity gets created when a post is updated.
        """
        reset_db()
        p = create_post()
        ...
        sa = SiteActivity.objects.all()[number_of_activities-1]
        self.assertEqual(number_of_activities, expected, 'Expected %s activity, got %s' % (expected, number_of_activities))
        self.assertEqual(sa.post_id, expected_id, 'Expected matching ids, got %s vs %s' % (expected_id, sa.post_id) )
        self.assertEqual(sa.post_title, expected_title, 'Expected matching titles, got %s vs %s' % (expected_title, sa.post_title))
        self.assertEqual(sa.task, expected_type, 'Expected "%s" task, got %s' % (expected_type, sa.task) )

    def test_activity_with_type_deleted_is_created_when_post_is_deleted(self):
        """
        Test a "Deleted" type activity gets created when a post is deleted.
        """
        reset_db()
        p = create_post()
        expected_title = p.title
        ...
        number_of_activities = SiteActivity.objects.count()
        expected_id = p.id
        expected_type = 'Deleted'
        sa = SiteActivity.objects.all()[number_of_activities-1]
        self.assertEqual(number_of_activities, expected, 'Expected %s activity, got %s' % (expected, number_of_activities))
        self.assertEqual(sa.post_title, expected_title, 'Expected matching titles, got %s vs %s' % (expected_title, sa.post_title))
        self.assertEqual(sa.task, expected_type, 'Expected "%s" task, got %s' % (expected_type, sa.task) )

    def test_activity_with_type_comment_is_created_when_a_post_is_commented_on(self):
        """
        Test an "Comment" type activity gets created when a post is commented on.
        """
        reset_db()
        p = create_post()
        c = Client()
        ...
        expected_title = p.title
        expected = 3  # create, update and comment
        number_of_activities = SiteActivity.objects.count()
        expected_id = p.id
